    ...
    def inspect(self):
        ...
        if self.operation[0] == '*':
            if self.operation[1] == 'old':
                self.items[0] = divmod(int(self.items[0]) * int(self.items[0]), modulo)[1]
            else:
                self.items[0] = divmod(int(self.items[0]) * int(self.operation[1]), modulo)[1]
        else:
            if self.operation[1] == 'old':
                self.items[0] = divmod(int(self.items[0]) + int(self.items[0]), modulo)[1]
            else:
...
# 1000 Rounds, iterating over the monkeys and making them do their actions
n = 0
while n < 10000:
    m = 0
